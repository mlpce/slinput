...
static int completion_request(SLINPUT_State *state,
  ...
  if (string_length == 1 && string[0] == 'u') {
    SLINPUT_CompletionReplace(state, SLI_CHAR_STRL("up"));
  } else if (string_length == 1 && string[0] == 'd') {
    SLINPUT_CompletionReplace(state, SLI_CHAR_STRL("down"));
  } else {
...
int main(int argc, char **argv) {
  ...
  while (result > 0) {
    result = SLINPUT_Get(state, SLI_CHAR_STRL("> "), NULL,
      sizeof(buffer)/sizeof(buffer[0]), buffer);
